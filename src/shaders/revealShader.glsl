...
void main() {
  ...
  offX += snoise3(vec3(offX, offY, time) * 2.) * .3;
  offY += snoise3(vec3(offX, offX, time * 0.3)) * .1;
  float nc = (snoise3(vec3(offX, offY, time * .5) * 0.01)) * progressHover;
  float nh = (snoise3(vec3(offX, offY, time * .5 ) * 0.2)) * .3;

  nh *= smoothstep(nh, 0.1, 0.1);
  ...
  vec4 hover = texture2D(u_map, uv + vec2(nc + nh) * progressHover * (1. - progress));
